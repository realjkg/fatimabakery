...
test('delivery outside current service ZIPs is rejected before forwarding', async () => { const res = await normalizeAndValidate({ ...base, fulfillment: 'delivery', delivery_address1: '1 Main', delivery_city: 'Austin', delivery_state: 'TX', delivery_zip: '78701' }, { TEST_GEOCODER_STATUS: 'outside' }); assert.equal(res.ok, false); assert.equal(res.error.status, 'delivery_unavailable'); assert.equal(res.error.offer_pickup, true); assert.match(res.error.message, /Santa Rita Ranch area/); });
test('delivery malformed or unlisted ZIPs are rejected', async () => {
  for (const delivery_zip of ['', '7864', '78643']) {
    const res = await normalizeAndValidate({ ...base, fulfillment: 'delivery', delivery_address1: '1 Main', delivery_city: 'Liberty Hill', delivery_state: 'TX', delivery_zip });
    assert.equal(res.ok, false);
    assert.equal(res.error.status, 'delivery_unavailable');
  }
});
