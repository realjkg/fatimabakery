...
test('delivery boundary is enforced before order side effects and frontend includes customer message', () => {
  const msg = 'Delivery is currently limited to the Santa Rita Ranch area near Liberty Hill, Leander, and Georgetown. Please choose Friday pickup, or contact us before placing a delivery order.';
  assert.match(code, /CURRENT_DELIVERY_ALLOWED_ZIPS = \["78642", "78641", "78628"\]/);
  assert.match(code, /CURRENT_DELIVERY_BLOCKED_ZIP_PREFIXES = \["787"\]/);
  const handleOrder = code.match(/function handleOrder[\s\S]*?var orderId    = "FB-"/)?.[0] ?? '';
  assert.match(handleOrder, /validateDeliveryServiceArea_\(data\)/);
  assert.doesNotMatch(handleOrder, /evaluateProductionInventoryForOrder_/);
  assert.equal(handleOrder.indexOf('validateDeliveryServiceArea_(data)') < handleOrder.indexOf('var orderId'), true);
  assert.match(orderHtml, new RegExp(msg.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
});
