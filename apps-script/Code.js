...
var CURRENT_DELIVERY_ALLOWED_ZIPS = ["78642", "78641", "78628"];
var CURRENT_DELIVERY_BLOCKED_ZIP_PREFIXES = ["787"];
...
function extractDeliveryZip_(data) {
  data = data || {};
  var candidates = [data.delivery_zip, data.zip, data.postal_code, data.delivery_address, data.delivery_address1, data.delivery_city];
  for (var i = 0; i < candidates.length; i++) {
    var match = String(candidates[i] == null ? "" : candidates[i]).match(/\b\d{5}(?:-\d{4})?\b/);
    if (match) return match[0].substring(0, 5);
  }
  return "";
}
function isSupportedDeliveryZip_(zip) {
  zip = String(zip || "").trim().substring(0, 5);
  if (!/^\d{5}$/.test(zip)) return false;
  for (var i = 0; i < CURRENT_DELIVERY_BLOCKED_ZIP_PREFIXES.length; i++) {
    if (zip.indexOf(CURRENT_DELIVERY_BLOCKED_ZIP_PREFIXES[i]) === 0) return false;
  }
  return CURRENT_DELIVERY_ALLOWED_ZIPS.indexOf(zip) > -1;
}
function deliveryServiceAreaError_(data) {
  if (!isDeliveryOrder_(data)) return "";
  var hay = [data.delivery_address1, data.delivery_address2, data.delivery_city, data.delivery_state, data.delivery_zip, data.delivery_address, data.preferred_time].join(" ").toLowerCase();
  var zip = extractDeliveryZip_(data);
  if (hay.indexOf("austin") > -1) return DELIVERY_SERVICE_AREA_MESSAGE;
  if (!zip || !isSupportedDeliveryZip_(zip)) return DELIVERY_SERVICE_AREA_MESSAGE;
  return "";
}
function validateDeliveryServiceArea_(data) {
  var msg = deliveryServiceAreaError_(data);
  if (msg) return jsonResponse({ status: "delivery_unavailable", message: msg, offer_pickup: true });
  return null;
}
...
function handleOrder(data, ss) {
  ...
  var deliveryBoundaryFailure = validateDeliveryServiceArea_(data);
  if (deliveryBoundaryFailure) return deliveryBoundaryFailure;
  if (isDelivery && (!deliveryAddress.address1 || !deliveryAddress.city || !deliveryAddress.state || !deliveryAddress.zip)) {
    return jsonResponse({ status: "missing_address", message: DELIVERY_SERVICE_AREA_MESSAGE });
  }
  if (pickupDate) {
